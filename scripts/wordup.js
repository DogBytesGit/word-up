...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
		 ...
		 if (!element.hasOwnProperty("isRealWord")){
			element.isRealWord = theAnswer;
		 }
		});
...
function render() {
    ...
    var playerWords = model.wordSubmissions.map(wordSubmissionChip);
    $("#word-submissions").append(playerWords);
